...
If filename is None or the boolean == False,  filename is immediatly returned, otherwise new filename will be interpreted  over the mrbob render_filename function.
...
If you don't specify a  `-r, --rdr-fname-plugin-target`  NeitherBarOrFooInFilename will be loaded due to its order attribute, the higher is prefered.
